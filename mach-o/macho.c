...
#define for_each_lc(lcp, head) \
	for (uint32_t XXi = 0; XXi < head->ncmds; \
		XXi++, lcp = (struct load_command*)add_ptr2(lcp, lcp->cmdsize))

#define for_each_section(sect, seg) sect = \
	(struct section*)((uintptr_t)seg + sizeof(struct segment_command)); \
...
static void _map_section(mach_loader_context_t* file, struct section* sect, uint8_t* load_addr)
{
	...
	printf("Info: _map_section: va: 0x%08x size: 0x%x\n", sect->addr, sect->size);

	if ((sect->flags & SECTION_TYPE) == S_ZEROFILL) {
		/* This is a BSS section */
		memset((void*)va, 0, sect->size);
	}
	else {
		/* Copy the section from the source file */
		memmove((void*)va, (const void*)add_ptr2(file->file, sect->offset), sect->size);
	}
}
...
void _symtab_dump(mach_loader_context_t* file)
{
	for (uint32_t i = 0; i < file->symtab->nsyms; i++)
	{
		...
		printf("Info: [sym]: 0x%08x\n", sym->n_value);
	}
}

bool mach_file_is_prelinked(mach_loader_context_t* file)
{
		...
		if (lcp->cmd == LC_SEGMENT)
		{
			...
			if (file->filetype == MH_EXECUTE)
			{
				...
				if (cmd->filesize) {
					...
					memmove(dst, src, cmd->filesize);

					printf("Info: [MAP:%s]: copy %p =(%u)=> %p\n",
						cmd->segname,
						src,
						cmd->filesize,
						dst);
...
_relocate_sect(mach_loader_context_t* file, struct segment_command* seg, uint32_t ordinal)
{
	...
	printf("Info: _relocate_sect: %d %s\n", ordinal, sect->sectname);
