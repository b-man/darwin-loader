...
static int parse_data_array(jsmntok_t* tokens, size_t* out_len, void** out_data, Context* ctx) {
	...
	while (nested != 0) {
		...
		if (token->type == JSMN_STRING) {
			...
			memmove(&buf[cnt], &(ctx->raw[token->start]), len);
...
static void* token_to_string(jsmntok_t* token, Context* ctx, size_t* out_len)
{
	...
	memmove(buf, &(ctx->raw[token->start]), len);
	buf[len] = '\0';
...
int parse_jsdt_device_tree(const char* raw) {
	...
	printf("Info: parsing JSDT device tree at 0x%08x ...\n", (unsigned int)raw);
...
parse_again:
	...
	if (err == 0) {
		...
		if (ret) {
			/* SUCCESS */
			printf("Done: loaded JSDT device tree with %u nodes\n", ctx.node_count);
		}
		else {
			/* FAIL */
			printf("Error: malformed JSDT\n");
		}
	}
	else {
		printf("Error: parse error in JSDT\n");
		ret = 0;
	}
