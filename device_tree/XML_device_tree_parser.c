...
static void* ArrayToDeviceTreeData(TagPtr tag, uint32_t* len)
{
	...
	while (next) {
		if (next->type == kTagTypeInteger) {
			...
			panic("unknown type %ld", next->type);
		}
		next = next->tagNext;
	}
	...
	while (next) {
		if (next->type == kTagTypeInteger) {
			...
			*buf32 = (uint32_t)next->string;
			...
			memmove(&buf[cnt], next->string, len);
			cnt += len;
		}
		next = next->tagNext;
	}
...
/* main routine */
bool parse_xml_device_tree(uint32_t base)
{
	...
	printf("Info: parsing XML device tree at 0x%08x ...\n", base);
	...
	while (true)
	{
		...
		if (tag->type == kTagTypeDict) {
			...
			printf("Info: loaded XML device tree with %u nodes\n", ctx.node_count);
	...
	printf("Error: root dictionary not found in the XML device tree\n");
