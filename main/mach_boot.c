...
void flatten_device_tree(memory_range_t* range)
{
	...
	*((uint32_t*)dt_base) = kDeviceTreeMagic;
	...
	printf("Info: DTRE: flattening (0x%p) ...\n", dt_base);
...
bool allocate_memory_range(Node* memory_map, char * rangeName, long start, long length, long type)
{
	char *nameBuf;
	uint32_t *buffer;
...
bool enter_memory_range(Node* memory_map, char* prefix, uint32_t id, uint32_t type, memory_range_t* range)
{
	char* range_name;
	if (id != 0) {
		char range_buf[32];
...
bool map_add_ramdisk(Node* memory_map, memory_range_t* ramdisk_range)
{
	printf("Info: adding ramdisk [0x%08x, sz=0x%08x] to mem map\n",
		ramdisk_range->base,
		ramdisk_range->size);

	return
	enter_memory_range(memory_map, "RAMDisk",
		0,
		kBootDriverTypeInvalid,
		ramdisk_range);
}

static bool map_booter_extension(Node* memory_map, loaded_driver_image_t* image)
{
	/*
	...
	if ((sizeof(*driver)+NAME_LEN) > DRIVER_PAD_START) {
		printf("Error: DRIVER_PAD_START is too small. please fix this.\n");
		return false;
	}
	...
	if (!image->info_offset && image->has_exec) {
		printf("Error: driver %s has no Info.plist\n", (const char*)(&image->name));
		return false;
	}
	...
	memmove((void*)bundle_name, (void*)&image->name, NAME_LEN);

	driver->bundlePathAddr = (void*)(bundle_name);
	driver->bundlePathLength = strlen(bundle_name);

	if (strncmp(driver->plistAddr, "<?xml", 5) != 0) {
		printf("Warning: %s has a strange info.plist (starts with %.*s)\n",
			bundle_name,
			5,
			driver->plistAddr);
	}

	printf("Info: KEXT: %s E[%p 0x%lx] I[%p 0x%lx]\n",
		bundle_name,
...
bool map_add_drivers(Node* memory_map)
{
	...
	if (!next) {
		printf("Info: no kexts are loaded\n");
		return true;
	}
...
bool map_add_info(Node* memory_map,
	...
	memory_range_t* kernel_range)
{
	...
	ret = enter_memory_range(memory_map,
		"iBoot",
		0,
		kBootDriverTypeInvalid,
		&iBoot_range);
...
static int mach_boot(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (!gHasDeviceTree) {
		printf("Error: device tree is not loaded - load one before starting the kernel\n");
		return 1;
	}

	/*---------------------------------------------------------------*/

	printf("Info: kmem start=0x%08x size=0x%08x\n",
	...
	/*---------------------------------------------------------------*/
	/**** boot args ****/
	printf("Info: BOOT: allocating boot args ...\n");
	args = allocate_boot_args(&r_boot_args);

	/*---------------------------------------------------------------*/
	/**** MemoryMap ****/
	printf("Info: BOOT: init memory map ...\n");

	memory_map = create_memory_map();
	assert(memory_map);

	ret = map_add_info(memory_map,
		&r_boot_args,
		&gKernelMemoryRange);

	if (!ret) {
		printf("Error: map_add_info\n");
		goto out_err;
	}
	...
	printf("Info: DTRE: Final DT [%08x-%08x]\n",
		r_device_tree.base,
	...
	printf("Info: BOOT: populating boot args ...\n");

	printf("Info: phys_base=0x%08x virt_base=0x%08x\n",
		KERNEL_PHYS,
		KERNEL_VMADDR);
...
out_err:
	...
	printf("Done: starting kernel at 0x%08x ...\n", kernel_entry_point);
