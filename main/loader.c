...
#define kTableOfContentsMagic 0x43666F54 /* ((uint32_t)'CfoT') */
#define kCommandMachO 0x6863614D         /* ((uint32_t)'hcaM') */
#define kCommandXMLDeviceTree 0x54442d58 /* ((uint32_t)'TD-X') */
#define kCommandJSDeviceTree 0x5444534A  /* ((uint32_t)'TDSJ') */
#define kCommandRamdisk 0x4B534452       /* ((uint32_t)'KSDR') */
#define kCommandConfiguration 0x464E4F43 /* ((uint32_t)'FNOC') */
	...
	uint32_t* out_size)
{
	...
	printf("Info: vmsize=0x%x paddr=0x%x vaddr=0x%x\n", full_size, load_address, load_bias);
...
static bool assert_kernel_load(void)
{
	/*
	...
	if (RANGE_IS_NULL(gKernelMemoryRange)) {
		printf("Warning: a kernel image has to be loaded first\n");
		return false;
	}
	return true;
}
...
static void increment_kernel_memory(uint32_t by)
{
	...
	gKernelMemoryTop += by;
	env_set_hex("KernelMemoryTop", gKernelMemoryTop);
}
...
static int load_macho_from_command(command_macho_t* command)
{
	...
	if (command->name[NAME_LEN-1] != '\0') {
		printf("Warning: image name not NULL terminated - adding a NULL\n");
	...
	if (flags & kMachKernel) {
		...
		if (!RANGE_IS_NULL(gKernelMemoryRange)) {
			printf("Warning: a kernel is already loaded - tearing it down\n");
			teardown_old_loader_context();
		}

		if (CONFIG_NR_DRAM_BANKS < 1) {
			printf("Error: no DRAM banks available, can't allocate memory\n");
			return 1;
		}
	...
	printf("Info: macho@%08x: '%s' cmp=%d sz=%08x dst=%08x\n",
		image_address,
	...
	if (is_compressed) {
		...
		else if (flags & kCommandMachOFlags_CompressedQLZ)
		{
			...
			if (qlz_len != command->decomp_size) {
				printf("Error: QLZ decomp size mismatch (QLZ:0x%08x IMGX:0x%08x)\n",
					qlz_len,
					command->decomp_size);
			...
			if (!state_decompress) {
				printf("Error: unable to allocate %u bytes for QLZ decompressor\n",
					sizeof(qlz_state_decompress));

				return 1;
			}

			printf("Info: QLZ: 0x%08x => %p ...\n", image_address, decomp_image);
		...
		{
			printf("Error: Unrecognized compression type\n");
			return 1;
		}
	...
	if (flags & kMachDriver) {
		...
		if (command->info_offset > image_size) {
			printf("Error: Malformed load command (InfoOffset > ImageSize)");
			return 1;
		}
		...
		if (!is_compressed) {
			 ...
			 * the kernel memory.
			 */
			memmove((void*)raw_image_dest, (void*)image_address, image_size);
		}
		...
		/* Copy as bundle name */
		memmove((void*)&this->name, (void*)&command->name, NAME_LEN);
		...
		if (flags & kCommandMachOFlags_HasInfoPlist) {
			...
			if (flags & kCommandMachOFlags_NoExec) {
				printf("Error: NoExec driver has no info.plist");
				free((void*)this);
				return 1;
			}
			else {
		...
		if (flags & kCommandMachOFlags_NoExec) {
			printf("Done: loaded pure Info.plist driver '%s'\n", (const char*)&command->name);
		}
		else {
			printf("Done: loaded Info.plist/Exec driver '%s'\n", (const char*)&command->name);
		}
	}
		...
		if (!ret) {
			/* Failed to map */
			printf("Error: failed to map kernel\n");
			return 1;
		}
		...
		printf("Done: loaded kernel '%s' (ep=%08x)\n", (const char*)&command->name, entry_point);
	}
	else {
		printf("Error: unsupported mach-o type (want driver or kernel)\n");
		return 1;
	}
...
/* DT parsers */
extern bool parse_xml_device_tree(uint32_t base);
extern bool parse_jsdt_device_tree(uint32_t base);
...
static int parse_xdt_command(command_t* cmd)
{
	...
	if (gHasDeviceTree) {
		/* not fatal */
		printf("Warning: a device tree is already loaded, skipping\n");
		return 0;
	}
...
static int parse_table_of_contents(table_of_contents_t* toc)
{
	...
	printf("Info: toc@%08x: %u load commands\n", (uint32_t)toc, left_cmds);

	while (left_cmds--) {
		int ret = 0;

		if (cmd->magic == kTableOfContentsMagic) {
			printf("Error: ToC within a ToC is not allowed\n");
			ret = 1;
		}
			...
			printf("Error: load command 0x%08x is unknown\n", cmd->magic);
			ret = 1;
		}
...
static int load_general_image(uint32_t image_address, uint32_t image_size)
{
	...
	printf(
		"Info: image at 0x%08x, magic %c%c%c%c\n",
		image_address,
		cm[0],
		cm[1],
		cm[2],
	...
	if (image_magic == kCommandMachO) {
		...
		printf("Error: unknown image type (hex: 0x%08X)\n", image_magic);
		return 1;
	}
}
...
static int do_imgx(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
		...
		strcmp(argv[1], "l") == 0)
	{
		...
		if (!addr) {
			printf("Error: last address is NULL\n");
			return 1;
		}
	}
	else
...
static int do_rdx(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (!addr) {
		printf("Error: last loaded address is NULL\n");
		return 1;
	}
	if (!size) {
		printf("Error: last loaded filesize is NULL\n");
		return 1;
	}

	/* uhhh, uboot */
	if (addr != gKernelMemoryTop) {
		printf("Error: ramdisk loaded at the wrong address (use KernelMemoryTop env var)\n");
		printf("Error: (0x%08x instead of 0x%08x)\n", addr, gKernelMemoryTop);
		return 1;
	}

	/* validate the ramdisk */
	if (size < (1024 + sizeof(struct HFSPlusVolumeHeader))) {
		printf("Error: loaded ramdisk too small to be valid HFS+ dmg (0x%08x bytes)\n", size);
		return 1;
	}

	hdr = (struct HFSPlusVolumeHeader*)(addr+1024);

	if (__builtin_bswap16(hdr->signature) != kHFSPlusSigWord &&
		__builtin_bswap16(hdr->signature) != kHFSSigWord) {
		printf("Error: bad HFS+ signature (got 0x%08x wanted 'H+' or 'HX')\n", (uint32_t)__builtin_bswap16(hdr->signature));
		return 1;
	}
	...
	printf("Done: loaded dmg [0x%08x-0x%08x, %u files, %u dirs]\n",
		addr,
		addr+size,
		__builtin_bswap32(hdr->fileCount),
		__builtin_bswap32(hdr->folderCount));
